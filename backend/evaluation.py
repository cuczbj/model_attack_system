...
import models.metrics.fid
import utils
from models import inception
from models.classifiers.target_facenet64 import FaceNet

#1-计算 FID（Frechet Inception Distance）：用于衡量生成图像的质量。
def calc_fid(recovery_img_path, private_img_path, batch_size=64):
    ...
    mu_fake, sigma_fake = models.metrics.fid.calculate_activation_statistics(
        recovery_images, inception_model, batch_size, device=device
    )
    mu_real, sigma_real = models.metrics.fid.calculate_activation_statistics(
        private_images, inception_model, batch_size, device=device
    )
    fid_score = models.metrics.fid.calculate_frechet_distance(
        mu_fake, sigma_fake, mu_real, sigma_real
    )

    return fid_score

#提取私有数据特征 (get_private_feats),并存储为 .npy 文件，以供后续计算 KNN 使用
...
#2-计算 KNN 距离 (calc_knn & get_knn_dist)
...
# 测试
def test_evaluation(all_images_tensor):
    """
    测试评估函数
    
    """
    # Load evaluation model(固定为Facenet,最好还是调研一下其他方法的评估模型)
    E = FaceNet(1000)
    ...
    print("shape of all_images_tensor: ", all_images_tensor.shape)
    print("Evaluating...")

    # 评估每张图像
    predictions = []
    with torch.no_grad():
        for i in range(all_images_tensor.shape[0]):
            img = all_images_tensor[i].unsqueeze(0)  # 添加 batch 维度
            pred = E(augmentation.Resize((112, 112))(img))[-1].argmax(dim=1).item()
            predictions.append(pred)

    print("Predictions for all images:", predictions)
    # KNN 距离计算测试
