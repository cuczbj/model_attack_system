...
"""损失函数模块，包括判别器（Discriminator）和生成器（Generator）的不同损失函数计算方法。此外，
还提供了一些自定义损失，如 最大边际损失（Max Margin Loss） 和 Poincaré Loss，适用于特定任务。
"""
