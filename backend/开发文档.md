...
使用watchdog监视传入的模型架构文件，额后面不用这个组件了了
...
```
D:\workshop\model_attack_system\backend>curl -X POST http://localhost:5000/predict -F "image_file=@test_MLP_ATT1.png" -F "model_name=target_mlp" -F "param_file=MLP_50.pkl" -F "class_num=40"
{
  "confidences": [
    0.02415965311229229,
...
先完成上传功能的判断后缀确定上传位置，ok

嗯。。，再来进行可以访问的目标模型（为了不用再攻击时传入模型参数文件的位置），把两个get化为1个：
...
```
C:\Users\wrwut>curl -X GET http://localhost:5000/models
{
  "checkpoints": [
    "FaceNet64_88.50.tar",
    "IR152_91.16.tar",
  ...
  "matched_models": {
    "target_facenet64": "FaceNet64_88.50.tar",
    "target_iri152": "IR152_91.16.tar",
    "target_vgg16": "VGG16_88.26.tar"
  },
  "models": {
    "target_facenet64": "FaceNet64",
    "target_iri152": "IR152",
...
看那个matched_models就行，不过这样的话进行攻击前得进行一次/models接口的访问，OK的
...
这里我规定class_num输出维度是使用参数传入，后面可用规定在文件名里固定位置写再提取，但是输入维度要怎么处理呢，因为攻击的时候要生成一个同等大小可输入模型的噪声图像
...
curl -X POST http://127.0.0.1:5000/attack ^
     -H "Content-Type: application/json" ^
     -d "{\"target_label\": 5, \"attack_method\": \"standard_attack\", \"target_model\": \"target_mlp\", \"class_num\": 40, \"image_size\": \"112*92\"}"

```

这里这个没有权限访问checkpoints是？，好吧，发现没有对应的文件，因为还没有访问接口models，没有将其放入matched_models队列中,

```
2025-03-25 01:19:37,520 - werkzeug - INFO - 127.0.0.1 - - [25/Mar/2025 01:19:37] "GET /models HTTP/1.1" 200 -
Value of w: 92
key is target_facenet64
value is FaceNet64_88.50.tar
key is target_iri152
value is IR152_91.16.tar
key is target_mlp
value is MLP_50.pkl
param_file is MLP_50.pkl
D:\workshop\model_attack_system\backend\upload_importlib.py:62: FutureWarning: You are using `torch.load` with `weights_only=False` (the current default value), which uses the default pickle module implicitly. It is possible to construct malicious pickle data which will execute arbitrary code during unpickling (See https://github.com/pytorch/pytorch/blob/main/SECURITY.md#untrusted-models for more details). In a future release, the default value for `weights_only` will be flipped to `True`. This limits the functions that could be executed during unpickling. Arbitrary objects will no longer be allowed to be loaded via this mode unless they are explicitly allowlisted by the user via `torch.serialization.add_safe_globals`. We recommend you start setting `weights_only=True` for any use case where you don't have full control of the loaded file. Please open an issue on GitHub for any issues related to this experimental feature.
  model.load_state_dict(torch.load(param_file, map_location=device))
2025-03-25 01:19:46,113 - root - DEBUG - 开始执行攻击 方法:standard_attack 目标:5 任务ID:task-d27a85d2
执行攻击方法: standard_attack, 目标标签: 5, 任务ID: task-d27a85d2
Performing Attack: 100%|███████████████████████████████████████████████████████████████████| 5000/5000 [00:25<00:00, 197.44it/s] 
攻击完成，结果已保存至: ./result/attack/task-d27a85d2_5.png
攻击完成，已保存图像文件和返回base64数据。
2025-03-25 01:20:11,486 - root - DEBUG - 攻击完成，结果类型: <class 'str'>
2025-03-25 01:20:11,498 - root - DEBUG - 结果是base64字符串，长度: 26932
2025-03-25 01:20:11,518 - werkzeug - INFO - 127.0.0.1 - - [25/Mar/2025 01:20:11] "POST /attack HTTP/1.1" 200 -
2025-03-25 01:21:14,223 - werkzeug - INFO -  * Detected change in 'D:\\workshop\\model_attack_system\\backend\\test\\target_vgg16.py', reloading
2025-03-25 01:21:14,404 - werkzeug - INFO -  * Detected change in 'D:\\soft\\Anaconda\\Anaconda\\envs\\MAS\\Lib\\site-packages\\torch\\nn\\modules\\batchnorm.py', reloading
2025-03-25 01:21:14,597 - werkzeug - INFO -  * Detected change in 'D:\\soft\\Anaconda\\Anaconda\\envs\\MAS\\Lib\\site-packages\\torch\\nn\\parameter.py', reloading
2025-03-25 01:21:15,762 - werkzeug - INFO -  * Restarting with watchdog (windowsapi)
2025-03-25 01:21:21,178 - werkzeug - WARNING -  * Debugger is active!
2025-03-25 01:21:21,192 - werkzeug - INFO -  * Debugger PIN: 168-203-860
2025-03-25 01:21:25,774 - werkzeug - INFO -  * Detected change in 'D:\\workshop\\model_attack_system\\backend\\test\\target_mlp.py', reloading
2025-03-25 01:21:28,156 - werkzeug - INFO -  * Restarting with watchdog (windowsapi)
2025-03-25 01:21:33,659 - werkzeug - WARNING -  * Debugger is active!
2025-03-25 01:21:33,675 - werkzeug - INFO -  * Debugger PIN: 168-203-860
```
...
curl -X POST http://127.0.0.1:5000/attack ^
     -H "Content-Type: application/json" ^
     -d "{\"target_label\": 5, \"attack_method\": \"standard_attack\", \"target_model\": \"target_vgg16\", \"class_num\": 1000, \"image_size\": \"64*64\"}"
```

MLP可接受【1，1，h, w】(准确应该为[1，h*w],但是前面加多层1不影响),VGG16接受【1，c,  h, w】，关键在于多处：

```
aim_flatten = torch.zeros(1, 3, h , w).to(device).requires_grad_() # 这个3是1还是3得看其是黑白还是彩色
```
...
1-模型实例化要有class_num传入

2-为了支持基础攻击，forward的第二个才是logits
...
模型架构py文件放到 `models/classifier/`里面:

````
target_分类器名称.py
````

模型参数文件(.pth,.tar,.pkl)放到 `checkpoints/target_mdoel/`里面：
...
$curl -X POST http://127.0.0.1:5000/attack ^
     -H "Content-Type: application/json" ^
     -d "{ \"target_label\": 5, \"attack_method\": \"PIG_attack\", \"target_model\": \"target_vgg16\", \"dataset\": \"celeba\", \"class_num\": 1000, \"image_size\": \"64*64\", \"channels\": 3 }"

# 后端显示
Seed 4, Step 1200: Loss = -0.3856, LR = 3.72e-03
Seed 4, Step 1300: Loss = -0.3641, LR = 2.78e-03
Seed 4, Step 1400: Loss = -0.5472, LR = 2.20e-03
Seed 4, Step 1500: Loss = -0.2936, LR = 2.00e-03
攻击完成，结果已保存
2025-04-02 16:29:36,628 - root - DEBUG - 攻击完成，结果类型: <class 'NoneType'>
2025-04-02 16:29:36,653 - werkzeug - INFO - 127.0.0.1 - - [02/Apr/2025 16:29:36] "POST /attack HTTP/1.1" 500 -
```

ok，保存了，就是那个返回值和后端的问题了，添加：
...
```
shape of all_images_tensor:  torch.Size([5, 3, 64, 64])
Evaluating...
Predictions for all images: [5, 327, 122, 664, 122]
```

不对啊，为什么是这样的啊，就只有一个正确吗，

**如果 `eval_iden[i] == gt`，说明分类器成功识别了攻击样本**，执行：

- 计数 `cnt` +1。
- 重新生成图像 `best_img` 并存入 `success_dir`。

`torch.topk(eval_prob[i], 5)`: 取 `eval_prob[i]` **最高的 5 个类别**。

**如果 `gt`（真实类别）在 `top-5` 预测中，则 `cnt5 += 1`**。

那也不对啊，top1指标不对啊

不是int和tensor格式的标签的问题
...
要对相应的模型进行scan到py文件，要把那个每个PYTHON文件里定义MODEL_NAME变量，明天进行攻击为什么越来越差的查看那个的查看

输入值不对是因为1而不是（1，512）是没开E的eval模式
...
```sh
$D:\workshop\model_attack_system\backend\test>curl -X POST http://localhost:5000/api/datasets/upload-for-model -F "dataset_file=@celeba.zip" -F "dataset_name=celeba" -F "description=CelebA face dataset" -F "model_name=target_vgg16"
{
  "error": "\u4e0a\u4f20\u6570\u636e\u96c6\u5931\u8d25: \u5904\u7406\u6570\u636e\u96c6\u5931\u8d25: [Errno 28] No space left on device"
}

D:\workshop\model_attack_system\backend\test>curl -X POST http://localhost:5000/api/datasets/upload -F "dataset_file=@celeba.zip" -F "dataset_name=celeba" -F "description=CelebA face dataset"
{
  "class_count": 1000,
  "description": "CelebA face dataset",
  "format": "class_directories",
  "id": "dataset-ff15acf1",
  "image_count": 27018,
  "name": "celeba",
  "path": "./datasets\\dataset-ff15acf1",
  "upload_time": "2025-05-18 20:25:02"
}
  
------------------------------------------------------------------------------------------------------------
$D:\workshop\model_attack_system\backend\test>curl http://localhost:5000/api/datasets
[
  {
    "class_count": 1000,
    "description": "CelebA face dataset",
    ...
    "id": "dataset-ff15acf1",
    "image_count": 27018,
    "metadata": "{\"creation_date\": \"2025-05-18 20:25:02\", \"original_filename\": \"celeba.zip\"}",
    "model_name": null,
    "name": "celeba",
    "path": "./datasets\\dataset-ff15acf1",
    "upload_time": "2025-05-18 20:25:02"
  },
  {
    ...
    "id": "dataset-a7850ec3",
    "image_count": 0,
    "metadata": "{\"creation_date\": \"2025-05-18 20:19:48\", \"original_filename\": \"celeba_private_domain.zip\"}",
    "model_name": null,
    "name": "celeba",
    "path": "./datasets\\dataset-a7850ec3",
    "upload_time": "2025-05-18 20:19:48"
  },
  {
    "class_count": 1,
    "description": "vgg16\u6a21\u578b\u7684\u9690\u79c1\u6570\u636e\u96c6",
    "format": "class_directories",
    "id": "vgg16-dataset-7974fdba",
    "image_count": 0,
    "metadata": "{\"creation_date\": \"2025-04-17 16:26:33\", \"original_filename\": \"sjj.zip\", \"model_name\": \"vgg16\"}",
    "model_name": "vgg16",
    "name": "vgg16_dataset",
    "path": "./datasets\\vgg16-dataset-7974fdba",
    "upload_time": "2025-04-17 16:26:33"
  },
  {
    "class_count": 1,
    ...
    "id": "dataset-b83c754a",
    "image_count": 0,
    "metadata": "{\"creation_date\": \"2025-04-17 16:23:40\", \"original_filename\": \"sjj.zip\"}",
    "model_name": null,
    "name": "sjj",
    "path": "./datasets\\dataset-b83c754a",
    "upload_time": "2025-04-17 16:23:40"
  }
]
------------------------------------------------------------------------------------------------------------
#搞错了，应该是另一个上传接口：
$D:\workshop\model_attack_system\backend\test>curl -X POST http://localhost:5000/api/datasets/upload-for-model -F "dataset_file=@celeba.zip" -F "dataset_name=celeba" -F "description=CelebA face dataset" -F "model_name=target_vgg16"
{
  ...
  "description": "CelebA face dataset",
  "format": "class_directories",
  "id": "target_vgg16-dataset-2c137fc8",
  "image_count": 27018,
  "model_name": "target_vgg16",
  "name": "celeba",
  "path": "./datasets\\target_vgg16-dataset-2c137fc8",
  "upload_time": "2025-05-18 21:07:45"
}
#现在是可以了，成功了但是没结果啊
```

![image-20250518211356649](%E5%BC%80%E5%8F%91%E6%96%87%E6%A1%A3.assets/image-20250518211356649.png)
...
```
C:\Users\wrwut>curl -X POST http://localhost:5000/evaluations ^
More?   -H "Content-Type: application/json" ^
More?   -d "{\"dataset\": \"celeba\", \"targetModel\": \"target_vgg16\", \"attackMethods\": [\"PIG_attack\"], \"labelRange\": { \"start\": 0, \"end\": 1 }, \"samplesPerLabel\": 2, \"advancedSettings\": { \"iterations\": 1000, \"learningRate\": 0.01, \"useRegularization\": true, \"batchSize\": 32 }}"
```

```
curl -X GET http://localhost:5000/evaluations/res/bd1c064e-acc9-40d2-b006-707419319e26
```
...
``` 
1.那个数据集信息获取的where id 改为where name
2.前端evaultions/page.tsx的对应api去掉“api/”
```
